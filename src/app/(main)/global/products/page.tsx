...
import {
  useGlobalProductGroups,
  useDeleteGlobalProductGroup,
} from '@/hooks/queries';
...
import { GlobalProductSearchTab } from '@/components/global/GlobalProductSearchTab';
import { ConfirmDialog } from '@/components/ui/confirm-dialog';

export default function GlobalProductGroupList() {
  const router = useRouter();
  const { data: groups = [], isLoading } = useGlobalProductGroups();
  const deleteGroup = useDeleteGlobalProductGroup();
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.success('삭제 완료', { description: '글로벌 상품 그룹이 삭제되었습니다.' });
    } catch {
      toast.error('삭제 실패', { description: '글로벌 상품 그룹 삭제에 실패했습니다.' });
    }
  };
  ...
  return (
    <PageContainer>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">글로벌 상품 관리</h1>
          <p className="text-muted-foreground mt-1">
            글로벌 상품 그룹과 개별 상품(내입 수량 포함)을 관리합니다.
          </p>
        </div>
        <button
          onClick={() => router.push('/global/products/new')}
        ...
        <TabsContent value="groups">
          {isLoading ? (
            ...
            <div className="py-16 text-center border-2 border-dashed rounded-xl text-gray-400">
              ...
              <p className="font-medium text-gray-500">등록된 글로벌 상품 그룹이 없습니다.</p>
              ...
              <table className="w-full text-sm">
                  ...
                  <tr>
                    ...
                    >
                      <span className="inline-flex items-center gap-1 justify-end">
                        상품 수 <SortIcon column="productCount" />
                      </span>
                    </th>
                    <th
                      className="px-4 py-3 text-right font-medium text-gray-600 cursor-pointer hover:text-gray-900 select-none"
                ...
                <tbody className="divide-y">
                  {sorted.map((group) => (
                    <tr
                      key={group.id}
                      onClick={() => router.push(`/global/products/${group.id}`)}
      ...
      <ConfirmDialog
        open={!!deleteTarget}
        title="글로벌 상품 그룹 삭제"
        description="글로벌 상품 그룹을 삭제하시겠습니까?"
        confirmLabel="삭제"
        variant="danger"
