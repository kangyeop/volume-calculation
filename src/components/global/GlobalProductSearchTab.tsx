...
import { useAllGlobalProducts } from '@/hooks/queries';
import { Search, ArrowUpDown, ArrowUp, ArrowDown, Loader2, Package } from 'lucide-react';

type SortKey = 'name' | 'sku' | 'globalProductGroupName' | 'innerQuantity' | 'createdAt';
type SortDirection = 'asc' | 'desc';

export function GlobalProductSearchTab() {
  const { data: products = [], isLoading } = useAllGlobalProducts();
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <SortableHeader column="innerQuantity" label="내입수량" onSort={handleSort} icon={<SortIcon column="innerQuantity" />} className="text-right" />
                <SortableHeader column="globalProductGroupName" label="그룹" onSort={handleSort} icon={<SortIcon column="globalProductGroupName" />} />
                ...
                <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-gray-500">
                    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs bg-gray-100 text-gray-700">
                      {product.globalProductGroupName ?? '-'}
                    </span>
                  </td>
