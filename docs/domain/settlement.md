...
기존 확정된 출고(SHIPMENT)의 패킹 결과를 복사(Copy)하여 point-in-time 스냅샷을 생성하고, 수동으로 박스를 지정하는 도메인. 자동 패킹 계산 기능은 제공하지 않는다.
...
| 상태 | 조건 | 의미 |
|------|------|------|
| `matched` | boxId != null | 기존 출고에서 매칭 + 박스 복사됨 |
| `matched_unassigned` | boxId == null, items 비어있지 않음 | 매칭됐으나 원본에 boxId 없었음 |
| `unmatched` | boxId == null, items 비어있음 | 기존 출고에서 주문번호를 찾지 못함 |
...
| `packing_results` | 원본 packingResult에서 boxId 복사, 또는 미매칭 시 boxId=null |
...
| **Packing** | packingResults 구조 재활용. 단, 자동 패킹 계산은 제외 |
