...
**치수 적합성 검사**: 아이템과 박스의 치수를 각각 오름차순 정렬 후 대응하는 값을 비교한다. 정렬 비교이므로 회전을 암묵적으로 허용한다.

**유효 용량**: 박스 부피 × 0.9 (90%). 10%는 여유 공간.

**그리디 배정**:
1. 전체 아이템이 하나의 박스에 들어가면 → 가장 작은 적합 박스를 선택
2. 불가능하면 → 가장 큰 박스에 부피 기준으로 최대한 채우고, 남은 아이템으로 1번 반복
