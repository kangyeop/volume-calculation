...
출고 프로세스 전체를 담당하는 도메인. 엑셀 업로드 → Shipment/Order/OrderItem 생성 → 패킹 계산 → 결과 내보내기 흐름을 관리한다. 정산(Settlement) 도메인은 [settlement.md](settlement.md) 참고.
...
출고 또는 정산 작업의 최상위 컨테이너. 하나의 엑셀 업로드 또는 수동 생성 단위.

| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | UUID (PK) | |
| userId | UUID, nullable | 소유자 (auth.users) |
| name | varchar(255) | 출고/정산명 (자동 생성: `YYYYMMDD-N-파일명`) |
| type | enum | `SHIPMENT` (출고) / `SETTLEMENT` (정산) |
...
**용도:**
- `SHIPMENT`: 일반 출고 프로세스. 패킹 계산 후 박스 자동 배정
- `SETTLEMENT`: 정산 프로세스. 이미 패킹된 데이터 정리. 박스 수동 배정
...
패킹 계산 시 아이템들을 `orderIdentifier`(주문번호) 기준으로 그룹핑한다. 동일 주문번호의 아이템들은 하나의 그룹으로 묶여 독립적으로 박스에 배정된다.
...
| 방식 | 엔드포인트 | 설명 | 유형 |
|------|-----------|------|------|
| 출고 파일 업로드 | `POST /api/upload/shipment` | 파일 + 포맷 선택 → Shipment(type=SHIPMENT) 자동 생성 → 아이템 즉시 저장 | SHIPMENT |
| 정산 파일 업로드 | `POST /api/upload/settlement` | 파일 → Settlement(type=SETTLEMENT) 자동 생성 → 아이템 즉시 저장 | SETTLEMENT |
| 벌크 추가 | `POST .../order-items/bulk-with-file` | 기존 Shipment에 엑셀로 아이템 추가 | - |
| 단건 추가 | `POST .../order-items` | 수동으로 단건 아이템 추가 | - |

### 지원 포맷

**출고(SHIPMENT) 엑셀 업로드 시** 사용자가 드롭다운으로 3가지 고정 포맷 중 하나를 선택한다:
...
**정산(SETTLEMENT) 엑셀 업로드 시** 정산 형식만 지원
...
[settlement.md](settlement.md) 참고.
...
**목록 조회:**
- `GET /api/shipments?type=SHIPMENT` — 출고 목록 (type=SHIPMENT 필터)
- `GET /api/shipments?type=SETTLEMENT` — 정산 목록 (type=SETTLEMENT 필터)
...
| PATCH | `/api/shipments/{shipmentId}` | 이름/메모 변경 (`{ name }` 또는 `{ note }`) |
...
| Method | Endpoint | 설명 |
|--------|----------|------|
| POST | `.../packing/calculate` | 패킹 계산 (`{ boxGroupId }`) |
...
| 연관 도메인 | 관계 |
|-------------|------|
| **Product** | OrderItem.productId로 상품 치수 참조. |
...
| `src/lib/services/shipment.ts` | Shipment CRUD, 출고명 생성, cascade 삭제, type 필터 |
| `src/lib/services/settlement.ts` | Settlement 전용 ([settlement.md](settlement.md) 참고) |
...
| `src/lib/services/upload.ts` | 출고 엑셀 업로드 파이프라인 |
...
| 파일 | 역할 |
|------|------|
| `src/app/(main)/shipments/` | 출고 UI 페이지 (목록, 생성, 상세) |
