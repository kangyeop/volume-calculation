...
import { useOutboundBatch, useOutboundBatchOutbounds } from '@/hooks/queries';
...
export const OutboundDetail: React.FC = () => {
  ...
  const { data: batch, isLoading: batchLoading } = useOutboundBatch(batchId || '');
  const { data: outbounds = [], isLoading: outboundsLoading } = useOutboundBatchOutbounds(
    batchId || '',
  );
  ...
  const unmatchedItems = useMemo(() => outbounds.filter((o) => !o.productId), [outbounds]);
