...
export const BoxGroupList: React.FC = () => {
  ...
  const handleDelete = async (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    if (!window.confirm('박스 그룹을 삭제하시겠습니까? 그룹 내 모든 박스도 함께 삭제됩니다.')) return;
    try {
      await deleteGroup.mutateAsync(id);
      toast.success('삭제 완료', { description: '박스 그룹이 삭제되었습니다.' });
    } catch {
      toast.error('삭제 실패', { description: '박스 그룹 삭제에 실패했습니다.' });
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">박스 그룹</h1>
          <p className="text-muted-foreground mt-1">박스 그룹을 관리합니다.</p>
        </div>
        <button
          onClick={() => navigate('/boxes/new')}
          className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition-colors"
        >
          <Plus className="h-4 w-4" />새 박스 그룹
        </button>
      </div>
      ...
      {groups.length === 0 ? (
        <div className="py-16 text-center border-2 border-dashed rounded-xl text-gray-400">
          ...
          <p className="font-medium text-gray-500">등록된 박스 그룹이 없습니다.</p>
          <p className="text-sm mt-1">새 박스 그룹을 만들어보세요.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-medium text-gray-600">박스 수</th>
            ...
            <tbody className="divide-y">
              {groups.map((group) => (
                <tr
                  key={group.id}
                  onClick={() => navigate(`/boxes/${group.id}`)}
